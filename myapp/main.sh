...
REGISTRY=shariftest
LOCALIMAGE=$(docker-compose -f $WORKSPACE/myapp/docker-compose.yml images | awk '$1 ~ /docker-container-name/ { print $2 }')
LOCAL_TAG=$(docker-compose -f $WORKSPACE/myapp/docker-compose.yml images | awk '$1 ~ /docker-container-name/ { print $3 }')
REMOTE_IMAGE=test
REMOTE_TAG=04
